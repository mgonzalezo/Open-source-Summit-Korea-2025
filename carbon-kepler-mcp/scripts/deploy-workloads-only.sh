...
echo "  kubectl exec -n carbon-mcp deploy/carbon-mcp-server -- python -c \"from src.kepler_client import KeplerClient; kc = KeplerClient('http://kepler.kepler-system.svc.cluster.local:28282/metrics'); pods = kc.list_pods('demo-workloads'); import json; print(json.dumps(pods, indent=2))\""
echo ""
echo "To test a specific pod's watts:"
echo "  kubectl exec -n carbon-mcp deploy/carbon-mcp-server -- python -c \"from src.kepler_client import KeplerClient; kc = KeplerClient('http://kepler.kepler-system.svc.cluster.local:28282/metrics'); print(kc.get_pod_power_watts('high-power-cpu-burner-789756c966-2brnn', 'demo-workloads'))\""
echo ""
